...
namespace Fortest {
    ...
    template<typename Logger = AssertLogger>
    class Assert {
        int m_num_passed{};
        int m_num_failed{};
        std::shared_ptr<Logger> m_logger;  ///< internal logger
    ...
    public:
        explicit Assert(std::shared_ptr<Logger> logger = std::make_shared<Logger>())
